...
function addProduct($category, $title, $description, $price, $stock, $condition, $imageURL, $discount)
{
    ...
    try {
        ...
        $sql = "INSERT INTO Products (product_id, user_id, category, title, price,stock,description,imageUrl,conditio,discount) VALUES (?, ?, ?, ?, ?,?,?,?,?,?)";
            $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "ssssiisssi", $product_id, $user_id, $category, $title, $price, $stock, $description, $imageURL, $condition, $discount);
...
function editPost($product_id, $category, $title, $description, $price, $imageurl, $condition, $stack, $discount)
{
    session_start();
    include ('conn.php');
    $sql = "UPDATE Products SET category = ?, title = ?, description = ?, price = ?, imageUrl = ?, `conditio` = ?, stock = ?,discount = ? WHERE product_id = ?";
    if ($stmt = $conn->prepare($sql)) {
        $stmt->bind_param('ssssssiis', $category, $title, $description, $price, $imageurl, $condition, $stack, $discount, $product_id);
        if ($stmt->execute()) {
